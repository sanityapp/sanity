...
var completed = function(file,type) {
  ...
  if(type !== 'preview') completedImages.push(completed);
  ...
  fs.readdir(scannedPrefix, function(err,files) {
    for(var i = 0; i < files.length; i++) {
      fs.stat(files[i],function(err,stats) {
        if(stats.isFile() && files[i].indexOf(".png") != -1) completedImages.push(path.basename(files[i]));
      });
    };
  });
};
...
app.get('/api/completed', function (req, res) {
  var id = req.param('id');
  if(id == null || typeof(id) === "undefined") res.send(completedImages);
  else res.download(scannedPrefix + completedImages[id]);
});
